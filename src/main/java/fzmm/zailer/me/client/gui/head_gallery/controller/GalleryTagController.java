...
public class GalleryTagController {
    ...
    public void updateButton() {
        this.openTagsButton.setMessage(Component.translatable("fzmm.gui.headGallery.button.tags", this.includeTags.size() + this.excludeTags.size()));
        ...
        if (!MCH_RESOURCES.hasTags()) {
            ...
            this.openTagsButton.tooltip(textList.stream().map(ClientTooltipComponent::create).collect(Collectors.toList()));
        }
    }
    ...
    private Component overlayButtonText(MchTag tag) {
        boolean included = this.includeTags.contains(tag);
        if (included || this.excludeTags.contains(tag)) {
            return tag.text(included);
        } else {
            return Component.literal(tag.formattedName());
        }
    }

    private void overlayUpdate(ELabelComponent tagsOverlayLabel) {
        this.onChange();
        tagsOverlayLabel.text(Component.translatable("fzmm.gui.headGallery.label.tagsOverlay",
