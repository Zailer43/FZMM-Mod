...
public class TextFormatRainbowTab implements ITextFormatTab, IMemento {
    ...
    public void setupComponents(EFlowLayout rootComponent) {
        ...
        this.hue = SliderRow.setup(rootComponent, "hue", 1d, 0d, 1d, Float.class, 3, 0.025d, d -> {});
        this.brightness = SliderRow.setup(rootComponent, "brightness", 0.8d, 0d, 1d, Float.class, 3, 0.025d, d -> {});
        this.saturation = SliderRow.setup(rootComponent, "saturation", 1d, 0d, 1d, Float.class, 3, 0.025d, d -> {});
        this.hueStep = SliderRow.setup(rootComponent, "hueStep", 0.05d, config.minRainbowHueStep(), config.maxRainbowHueStep(), Float.class, 3, 0.005d, d -> {});
    }
    ...
    public void backup(ObjectOutputStream output) throws IOException {
        output.writeFloat((float) this.hue.parsedValue());
        output.writeFloat((float) this.saturation.parsedValue());
        output.writeFloat((float) this.brightness.parsedValue());
        output.writeFloat((float) this.hueStep.parsedValue());
    }
