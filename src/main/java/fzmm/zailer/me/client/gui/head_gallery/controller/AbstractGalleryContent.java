...
public abstract class AbstractGalleryContent {
    ...
    public static ItemStack toStack(MchHead head, Display display) {
        ...
        builder.setName(Text.translatable(translationHey + "name", head.name()).getString(), display.nameColor);

        if (display.tags && head.tags().isPresent() && head.tags().get().length > 0) {
            builder.addLore(Text.translatable(translationHey + "tags.title").getString(), display.loreColor);
            for (var tag : head.tags().get()) {
                builder.addLore(Text.translatable(translationHey + "tags.tag", tag.name()).getString(), display.loreColor);
            }
        }
        ...
        if (display.publishedAt && head.publishedAt() != null) {
            ...
            builder.addLore(Text.translatable(translationHey + "publishedAt", dateString).getString(), display.loreColor);
        }

        if (display.id && head.id() != null) {
            builder.addLore(Text.translatable(translationHey + "id", head.id()).getString(), display.loreColor);
        }
    ...
    public EItemComponent toComponent(MchHead head) {
        ...
        if (!this.display.enabled) {
            ...
            itemComponent.tooltip(Text.literal(head.name()));
        }
