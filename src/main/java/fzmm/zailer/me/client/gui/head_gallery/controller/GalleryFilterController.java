...
public class GalleryFilterController implements IMemento {
    ...
    public void setupComponents(EFlowLayout rootComponent) {
        ...
        advancedSearchButton.onPress(button -> {
            ...
            overlay.filters(this.textFilters, AbstractTextFilter.removeFilters(this.searchField.getText()));
    ...
    public void init() {
        ...
        this.filterButton.setContextMenuOptions(dropdown -> {
            for (var filter : this.filters) {
                Text message = filter.buttonText();
    ...
    protected List<MchHead> filterHeads() {
        String search = AbstractTextFilter.updateFilters(this.searchField.getText(), this.textFilters.keySet()).toLowerCase(Locale.ROOT);

        Stream<MchHead> filteredStream = this.entries.parallelStream();
        if (!search.isEmpty()) {
            filteredStream = filteredStream.filter(head -> StringUtils.containsIgnoreCase(head.name(), search));
        }
