...
import fzmm.zailer.me.client.gui.headgenerator.HeadComponentEntry;
import fzmm.zailer.me.client.gui.widgets.image.ImageButtonWidget;
import fzmm.zailer.me.client.gui.widgets.image.mode.SkinMode;
...
public class HeadGeneratorScreen extends BaseFzmmScreen {
    ...
    protected void tryAddComponentList(FlowLayout rootComponent) {
        this.tryAddComponentList(rootComponent, "head-list-image-option", this.getImageRow(SKIN_ID));
        this.tryAddComponentList(rootComponent, "head-list-image-mode", this.getEnumRow(SKIN_SOURCE_TYPE_ID));
//        this.tryAddComponentList(rootComponent, "head-list-search-field", this.getTextFieldRow(SEARCH_ID, false));
    }

    @Override
    @SuppressWarnings("ConstantConditions")
    protected void setupButtonsCallbacks(FlowLayout rootComponent) {
        this.setupImage(rootComponent, SKIN_ID, SKIN_SOURCE_TYPE_ID, SkinMode.NAME);
        this.setupTextField(rootComponent, SEARCH_ID, "");
        ImageButtonWidget imageButton = rootComponent.childById(ImageButtonWidget.class, this.getImageButtonId(SKIN_ID));
        FlowLayout headListLayout = rootComponent.childById(FlowLayout.class, HEAD_LIST_ID);
        this.checkNull(headListLayout, "flow-layout", HEAD_LIST_ID);
        imageButton.setOnValueChanged(button -> this.onLoadSkin(button, headListLayout));
    }

    //TODO
    private void onLoadSkin(ButtonWidget button, FlowLayout headListLayout) {
        ImageButtonWidget imageButton = (ImageButtonWidget) button;
        if (imageButton.hasNoImage())
            return;

        headListLayout.clearChildren();
        List<HeadData> headDataList = HeadGeneratorResources.getHeadTexturesOf(imageButton.getImage());
        List<Component> headEntries = headDataList.stream()
                .map(headData -> (Component) new HeadComponentEntry(headData, Sizing.fill(100), Sizing.fixed(32)))
                .toList();
        headListLayout.children(headEntries);
...
//            Text detailsId = Text.translatable("fzmm.gui.headGenerator.wait_seconds", seconds - i);
//            CompletableFuture.delayedExecutor(i, TimeUnit.SECONDS).execute(() -> this.updateButtons(buttonList, detailsId, false));
//        }
//
...
//    }
//
//    public void updateButtons(List<ButtonWidget> buttonList, Text detailsId, boolean active) {
//        for (var button : buttonList) {
//            button.active = active;
//            button.setMessage(detailsId);
//        }
//    }
//
//    @Override
