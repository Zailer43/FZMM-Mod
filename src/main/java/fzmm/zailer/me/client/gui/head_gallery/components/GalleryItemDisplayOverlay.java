...
public class GalleryItemDisplayOverlay extends OverlayContainer<EFlowLayout> {
    ...
    private void buildComponents(AbstractGalleryContent.Display values) {
        ...
        this.child.child(EComponents.label(Text.translatable("fzmm.gui.headGallery.option.display")));
    ...
    private Component buildPreview() {
        ...
        this.preview = EComponents.label(Text.empty());
        this.updatePreview();
    ...
    private Component buildBottomButtons() {
        ...
        result.child(
                EComponents.button(Text.translatable("fzmm.gui.button.saveConfig"))
                        .onPress(button -> {
                            config.enabled(this.showDisplay.isChecked());
                            config.tags(this.showTags.isChecked());
                            config.publishedAt(this.showPublishedAt.isChecked());
                            config.id(this.showId.isChecked());
    ...
    private CheckboxComponent getOption(String translationKey, int permission, boolean enabled) {
        Text text = Text.translatable("fzmm.gui.headGallery.option.display.overlay.show" + translationKey);
        if (!FzmmClient.MCH_RESOURCES.licenseDetected().hasPermission(permission)) {
            ...
            text = Text.translatable("fzmm.gui.headGallery.tier.missingPermission", text, minLicense.message());
        }
    ...
    public void updatePreview() {
        ...
        List<Text> tooltip = previewStack.getTooltip(Item.TooltipContext.DEFAULT, MinecraftClient.getInstance().player, TooltipType.BASIC);
    ...
    public AbstractGalleryContent.Display toDisplay() {
        return new AbstractGalleryContent.Display(
                this.showDisplay.isChecked(),
                this.showTags.isChecked(),
                this.showPublishedAt.isChecked(),
                this.showId.isChecked(),
