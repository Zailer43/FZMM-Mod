...
public class SelfCollectionFilter extends AbstractFilter {
    ...
    public CompletableFuture<ApiResponse<List<IMchMatcher>>> initOptions() {
        ...
        return this.collections().whenComplete((ignored, throwable) ->
                MinecraftClient.getInstance().execute(snackBar::close)
        );
    }

    @Override
    protected Text filterText() {
        return Text.translatable("fzmm.gui.headGallery.button.collections.self");
    }

    @Override
    protected Text missingOptions() {
        return Text.translatable("fzmm.gui.headGallery.option.collection.self.missing");
    }
    ...
    protected Text loadingAlertText() {
        return Text.translatable("fzmm.gui.headGallery.snack_bar.loading.collection.self");
    }
}
