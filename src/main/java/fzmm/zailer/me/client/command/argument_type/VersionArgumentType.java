...
public class VersionArgumentType implements ArgumentType<Pair<String, Integer>> {
    ...
    public static Pair<String, Integer> parse(String value) throws CommandSyntaxException {
        ...
        Component errorText = Component.translatable("commands.fzmm.version_argument.message", VERSIONS.getFirst().getFirst(), VERSIONS.getLast().getFirst());
    ...
    public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
        List<String> stringVersions = VERSIONS.stream().map(Pair::getFirst).toList();
    ...
    private static List<Pair<String, Integer>> initVersions() {
        ...
        result.add(new Pair<>("26.2", 4903));
        result.add(new Pair<>("26.1.2", 4790));
        result.add(new Pair<>("26.1.1", 4788));
        result.add(new Pair<>("26.1", 4786));

        result.add(new Pair<>("1.21.11", 4671));
        ...
        result.add(new Pair<>("1.20.4", LATEST_VERSION_WITH_NBT));
