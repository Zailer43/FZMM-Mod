...
public class ItemTooltipAppend {
    private static final WeakHashMap<Integer, Long> compoundHash = new WeakHashMap<>(50);
    ...
    private static void addNbtLength(ItemStack stack, TooltipContext context, List<Text> lines) {
        if (!FzmmClient.CONFIG.general.showItemSize() || !context.isAdvanced()) return;
        long stackSize;

        int hash = stack.getNbt() == null ? 0 : stack.getNbt().hashCode();
