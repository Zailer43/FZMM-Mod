...
public class PlayerStatue {
    ...
    public CompletableFuture<Void> generateStatues() {
        ...
        MinecraftClient.getInstance().execute(() -> this.notifyStart(isCancelled));
        return this.generate(isCancelled)
                .whenComplete((unused, throwable) -> MinecraftClient.getInstance().execute(() -> {
                    this.snackBar.close();
    ...
    private void notifyComplete() {
        SnackBarManager.getInstance().add(BaseSnackBarComponent.builder(SnackBarManager.PLAYER_STATUE_ID)
                ...
                .keepOnLimit()
                .title(Text.translatable("fzmm.snack_bar.playerStatue.successful.title"))
                .sizing(Sizing.fixed(220), Sizing.content())
                .mediumTimer()
                .startTimer()
                .build()
    ...
    public void updateStatus(StatuePart part, long delayMillis) {
        MinecraftClient.getInstance().execute(() -> {
            ...
            this.snackBar.updateTitle(Text.translatable("fzmm.snack_bar.playerStatue.loading.title"));
            this.snackBar.updateDetails(Text.translatable("fzmm.snack_bar.playerStatue.loading.details",
                    part.getName(),
                    this.partsGenerated,
